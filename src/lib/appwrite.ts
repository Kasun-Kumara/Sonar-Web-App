import { Client, Databases, Query, ID, type RealtimeResponseEvent } from 'appwrite';
import {
	PUBLIC_APPWRITE_ENDPOINT,
	PUBLIC_APPWRITE_PROJECT_ID,
	PUBLIC_APPWRITE_DB_NAME,
	PUBLIC_APPWRITE_COLLECTION_TEAMS,
	PUBLIC_APPWRITE_COLLECTION_SESSIONS,
	PUBLIC_APPWRITE_COLLECTION_ACTIVITY_LOGS,
	PUBLIC_APPWRITE_COLLECTION_REPORTS,
	PUBLIC_APPWRITE_COLLECTION_SETTINGS
...
const DB_ID = PUBLIC_APPWRITE_DB_NAME || 'devwatch_db';
const COL_TEAMS = PUBLIC_APPWRITE_COLLECTION_TEAMS || 'teams';
const COL_SESSIONS = PUBLIC_APPWRITE_COLLECTION_SESSIONS || 'sessions';
const COL_ACTIVITY_LOGS = PUBLIC_APPWRITE_COLLECTION_ACTIVITY_LOGS || 'activityLogs';
const COL_REPORTS = PUBLIC_APPWRITE_COLLECTION_REPORTS || 'reports';
const COL_SETTINGS = PUBLIC_APPWRITE_COLLECTION_SETTINGS || 'settings';

const HEARTBEAT_SEC = 15; // 15 seconds heartbeat interval

// ---- Teams ----
export async function getTeamByName(teamName: string): Promise<Team | null> {
	try {
		const res = await databases.listDocuments(DB_ID, COL_TEAMS, [Query.limit(500)]);
...
export async function getAllSessions(): Promise<Session[]> {
	try {
		const res = await databases.listDocuments(DB_ID, COL_SESSIONS, [Query.limit(100)]);
...
export async function upsertActivityLog(payload: HeartbeatPayload): Promise<void> {
	...
	if (existing.documents.length > 0) {
		...
		const elapsed =
			(new Date(payload.timestamp).getTime() - new Date(sync.lastStatusAt).getTime()) / 1000;
		...
		// Sync activity events from client
		...
		const writeWithTrim = async () => {
			const attempts = [
				() => JSON.stringify(sync),
				() => {
					if (sync.activityEvents)
						sync.activityEvents = sync.activityEvents.slice(-Math.ceil(sync.activityEvents.length / 2));
				...
				() => {
					if (sync.activityEvents) sync.activityEvents = sync.activityEvents.slice(-20);
					return JSON.stringify(sync);
				},
				() => {
	...
	try {
		...
		if (existing.documents.length > 0) {
			...
			for (const app of summary.apps) {
				sync.apps[app] =
					(sync.apps[app] || 0) + summary.duration / 1000 / Math.max(summary.apps.length, 1);
			}
			...
			const sync: ActivitySyncData = {
				...
				apps: Object.fromEntries(
					summary.apps.map((a) => [a, summary.duration / 1000 / Math.max(summary.apps.length, 1)])
				),
				files: summary.files,
				...
				offlinePeriods: [
					{ from: summary.offlineFrom, to: summary.offlineTo, duration: summary.duration / 1000 }
				]
			};
			await databases.createDocument(DB_ID, COL_ACTIVITY_LOGS, ID.unique(), {
				teamId,
...
export async function flushAllActivityLogs(): Promise<{ success: boolean; error?: string }> {
	try {
		let hasMore = true;
		while (hasMore) {
			const res = await databases.listDocuments(DB_ID, COL_ACTIVITY_LOGS, [Query.limit(100)]);
...
export async function setGlobalInternetRestriction(
	blocked: boolean
): Promise<{ success: boolean; error?: string }> {
	try {
		const res = await databases.listDocuments(DB_ID, COL_SETTINGS, [
